...
from editor.core import (
    add_object as core_add_object,
)
from editor.core import (
    export as core_export,
)
from editor.core import (
    remove_object as core_remove_object,
)
from editor.models import Level
...
class App(tk.Tk):
    def __init__(self, level_path: Path | None):
        ...
        ttk.Button(bar, text="Export…", command=self.export_dialog).pack(
            side="left", padx=(8, 0)
        )
        ...
        self.tree = ttk.Treeview(
            left, columns=("type", "x", "y"), show="headings", selectmode="browse"
        )
        ...
        ttk.Button(rm_bar, text="Remove Selected", command=self.remove_selected).pack(
            side="left"
        )
        ...
        self.type_cb = ttk.Combobox(
            ...
            values=["wall", "spawn", "enemy", "coin", "door"],
            state="readonly",
            width=12,
        )
        ...
        ttk.Entry(right, textvariable=self.x_var, width=8).grid(
            row=1, column=1, sticky="w"
        )

        ttk.Label(right, text="Y").grid(row=2, column=0, sticky="w")
        self.y_var = tk.IntVar(value=0)
        ttk.Entry(right, textvariable=self.y_var, width=8).grid(
            row=2, column=1, sticky="w"
        )

        ttk.Button(right, text="Add", command=self.add_from_form).grid(
    ...
    def export_dialog(self):
        ...
        out = filedialog.asksaveasfilename(
            title="Export to…", defaultextension=".json", filetypes=[("JSON", "*.json")]
        )
        if not out:
            return
        try:
